...
import sys
sys.path.append('/dssg/home/acct-umjpyb/umjpyb/shengyin/DAMC/Rotated_IoU')
from oriented_iou_loss import cal_giou_3d
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, pred_boxes: Dict, target_boxes: Dict):
        ...
        # tmp_match_cost = cost_mat[row_ind, col_ind].detach().cpu().numpy()
        # if (tmp_match_cost > 2).sum() > 0 :
        #     print(np.where(tmp_match_cost > 2, tmp_match_cost, 0))
        # threshold = np.nanpercentile(tmp_match_cost, q = 70)
        # final_match_cost = torch.tensor(np.where(tmp_match_cost >= threshold, tmp_match_cost, 0)).cpu()
        # match_cost.append(final_match_cost.sum() / ((final_match_cost != 0).sum()))
